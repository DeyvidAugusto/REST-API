...
from schemas.schema import PedidoCreateSchema, PedidoUpdateSchema, PedidoOutSchema, ClienteCreateSchema, ClienteUpdateSchema, ClienteOutSchema
...
def get_PedidoController(db: Session = Depends(get_db)):
    repository = PedidoRepository(db)
    service = PedidoService(repository)
    return PedidoController(service)

def get_ClienteController(db: Session = Depends(get_db)):
...
def listar_pedidos(controller: PedidoController = Depends(get_PedidoController)):
    return controller.listar_pedidos()

@app.get("/pedidos/{pedido_id}", response_model=PedidoOutSchema)
def ler_pedido(pedido_id: int, controller: PedidoController = Depends(get_PedidoController)):
    return controller.ler_pedido(pedido_id)

@app.post("/pedidos", response_model=PedidoOutSchema, status_code=201)
def criar_pedido(pedido_data: PedidoCreateSchema, controller: PedidoController = Depends(get_PedidoController)):
    return controller.criar_pedido(pedido_data)

@app.put("/pedidos/{pedido_id}", response_model=PedidoOutSchema)
def atualizar_pedido(pedido_id: int, update_data: PedidoUpdateSchema, controller: PedidoController = Depends(get_PedidoController)):
    return controller.atualizar_pedido(pedido_id, update_data)

@app.delete("/pedidos/{pedido_id}", status_code=204)
def deletar_pedido(pedido_id: int, controller: PedidoController = Depends(get_PedidoController)):
    controller.deletar_pedido(pedido_id)
    return None

@app.get("/clientes", response_model=List[ClienteOutSchema])
def listar_clientes(controller: ClienteController = Depends(get_ClienteController)):
    return controller.listar_clientes()

@app.get("/clientes/{cliente_id}", response_model=ClienteOutSchema)
def ler_cliente(cliente_id: int, controller: ClienteController = Depends(get_ClienteController)):
    return controller.ler_cliente(cliente_id)

@app.post("/clientes", response_model=ClienteOutSchema, status_code=201)
def criar_cliente(cliente_data: ClienteCreateSchema, controller: ClienteController = Depends(get_ClienteController)):
    return controller.criar_cliente(cliente_data)

@app.put("/clientes/{cliente_id}", response_model=ClienteOutSchema)
def atualizar_cliente(cliente_id: int, update_data: ClienteUpdateSchema, controller: ClienteController = Depends(get_ClienteController)):
    return controller.atualizar_cliente(cliente_id, update_data)

@app.delete("/clientes/{cliente_id}", status_code=204)
def deletar_cliente(cliente_id: int, controller: ClienteController = Depends(get_ClienteController)):
